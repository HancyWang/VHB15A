...
void SHT21_Start(void);
void SHT21_Stop(void);
u8t SHT2x_CheckCrc(u8t Cdata[], u8t nbrOfBytes, u8t checksum);
unsigned char SHT21_Write8Bit(unsigned char input);
unsigned char SHT21_Read8Bit(bit ask);
...
static unsigned char SHT21_Write8Bit(unsigned char i_input)
{
		...
		while(SDA_CONF)
		{
			if(++i>2400){SCL_OUTPUT_LOW();return(1);}
		}
		SCL_OUTPUT_LOW();
		return 0;
}
