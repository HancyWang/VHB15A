...
//}Wire_Mode_Sel;	
static WIRE_MODE_SEL Wire_Mode_Sel=Wire_Sel_Double;
		
//错误检测和报警相关变量	
uint8_t  ERR_Kind=0;//错误的种类//0传感器/1高温/2低温/3低湿/4无水/5患者传感器/6出气口传感器/7加热盘传感器   
uint8_t Wire_Mode_Mismatch ; //回路发热丝模式和实际不匹配		
static uint8_t JRP_Sensor_Err=0;
static uint8_t CQK_Sensor_Err=0;
static uint8_t RTD_Sensor_Err=0;
static uint8_t SHIDU_Sensor_Err=0;		
		
uint8_t AlarmSoundPauseStatus = 0;//报警声音暂停 0-正常报警 1-报警声音暂停(仅报警时有效)
uint8_t AlarmInfoIndex = 0;//
          
static uint8_t    HiTemp_Count=0;	  //高温度次数
static uint8_t    LoTemp_Count=0; 	  //低温度次数
static uint8_t    LoTemp_CQK_Count=0;//出气口低温度次数
static uint8_t    LoHumity_Count=0;  //低湿度次数
...
//实时参数
INT    					RT_Temp=0;            //人体端温度
INT    					JEP_Temp=280;           //加热盘温度
LONG    					CQK_Temp=280;           //出气口温度
static uint16_t    	RT_SHIDU=0;             //人体端湿度
static uint16_t    	CONTROL_RT_SHIDU=550;     //控制加热的湿度
...
//	int Uk1;//上次的总控制量
	...
	INT Ek1;//前一次的误差量
	INT Ek2;//前二次的误差量       
}Controler_temp,Controler_temp2,Controler_temp3;//定义一个温度Controler结构

//static CONTROLLER_TEMP Controler_temp;
//static CONTROLLER_TEMP Controler_temp2;
//static CONTROLLER_TEMP Controler_temp3;
...
// void Main_Init(void);	//得到存放数据的条数,画工作窗口,使能涓流充电,RTC检测,其它变量初始化
...
static void Controler_Calc(void); //Controler算法
static void HeatPlateTemp_Aim_Controler_Calc(void); //加热盘目标温度Controler算法---
static void HeatPlate_Controler_Calc(void);//加热盘的Controler算法---
...
void UART_RecData_Func(void)
{
	...
	while(EUSART_DataReady!=(uint8_t)0)//缓冲区有数据
	{
...
void HeaterWireModeDetFunc(void)
{
	...
	{
		if((WireIn_State == (uint8_t)0)||(WireOut_State == (uint8_t)0))	
		{
			Wire_Mode_Mismatch = (uint8_t)1;//回路发热丝模式和实际不匹配标志
		}
		else
		{
			Wire_Mode_Mismatch = (uint8_t)0;//回路发热丝模式和实际不匹配标志
		}
	}	
...
void RefreshTempHumidyFunc(uint8_t Refresh_En)//刷新显示温度和湿度
{
	...
	Refresh_WenDu_Cnt++;
	if(Refresh_WenDu_Cnt > (uint8_t)5)//1.2S
	{
		Refresh_WenDu_Cnt = (uint8_t)0;	
	}
	
	Alarm_WenDu_Cnt++;
	if(Alarm_WenDu_Cnt > (uint8_t)1)
	{
		Alarm_WenDu_Cnt = (uint8_t)0;
	}
...
//	Temp_Dis_En = 0;//温度显示标志
	...
	{
		if(Refresh_WenDu_Cnt == (uint8_t)0)
		{
			Temp_Dis_En = (uint16_t)1;//温度显示标志
		}
	}
		
	if(Temp_Dis_En == (uint16_t)1)
	{			
	...
	//显示湿度
	color=BLACK18;
	if((Bit_is_one(ERR_Kind,Alarm_Const_LoHumity))!=(uint8_t)0)   //低湿度报警
	{
		Temp_Dis_En = (uint16_t)1;//温度显示标志
		color=RED18;
		if(Alarm_WenDu_Cnt == (uint8_t)1)
...
//			Read_SCL_CONF=SCL_CONF;
		...
		if((Read_SHT21_Err!=(uint8_t)0)||(RT_SHIDU == (uint16_t)0))
		{	
			if(No_ReadData_SHIDU_Times<(uint8_t)100)
			{
				No_ReadData_SHIDU_Times++;
			}				
...
void  HeaterPlateWireControlFunc(void)//温度控制
{
	...
	{	//8表示高位为0也显示,0表示非叠加显示 出气口
		...
		LCD_ShowxNum(POS_RT_RH_X-50,34,16,4,(u32)JEP_Temp,0x80,BLACK18); //发热盘的温度
		LCD_ShowxNum(POS_RT_RH_X-50,73,16,3,(u32)CQK_Temp,0x80,BLACK18); //出气口的温度
		...
		LCD_ShowxNum(POS_RT_RH_X-33,2,16,1,(u32)Wire_Mode_Sel,0x80,BLACK18); //加热丝模式			
		...
		LCD_ShowxNum(POS_RT_RH_X-33,107,16,3,(u32)Set_CQK_Temp_Comp,0x80,BLACK18); //校正过后的出气口目标湿度
		...
		{
			LCD_ShowxNum(POS_RT_RH_X-33,34,16,4,(u32)Temp2_Int,0x80,BLACK18); //无发热丝回路时发热盘的目标温度
			LCD_ShowxNum(POS_RT_RH_X-16,34,16,3,NoneWire_Heat_Sec,0x80,BLACK18); //无发热丝回路时改变时间	
		}
			
		LCD_ShowxNum(POS_RT_RH_X+40,170,16,3,(u32)Temp1_Int,0x80,BLACK18); //开机渐进式温度目标
		LCD_ShowxNum(POS_RT_RH_X+20,170,16,3,(u32)RT_Temp,0x80,BLACK18); //患者端温度
	...
	if(		     
		((ERR_Kind & (uint8_t)0x01)!=(uint8_t)0)//传感器错误				
		||((ERR_Kind & (uint8_t)0x02)!=(uint8_t)0) //高温
		||((ERR_Kind & (uint8_t)0x10)!=(uint8_t)0) //无水		
	...
	{				
		...
		{//回路发热丝的控制
			if(WireIn_State!=(u8)0)		//当有管内IN发热丝时，管内发热丝的加热逻辑
			{
				...
				//Controler_temp.Ek = Set_RT_Temp - RT_Temp;//当前的误差量 设定温度-当前实际温度
				Controler_temp.Ek = Temp1_Int - RT_Temp;//当前的误差量 设定温度-当前实际温度
				Controler_Calc(); 
	...
	//										if(Work_Min>=20||Work_Day>0 ||Work_Hour>0)//开机20分钟之后
							...
							else
							{
								if(JEP_Temp_Aim > 980)
								{
									JEP_Temp_Aim = 980;//开机20分钟以内最大98度
								}
							}
						}
					}
					...
					Controler_temp2.Ek = JEP_Temp_Aim - JEP_Temp;//当前的误差量 设定温度-当前实际温度
					HeatPlate_Controler_Calc(); 
					Val_Calc = (uint8_t)Controler_temp2.Uk;	//计算出加热时间
...
static void   Store_Data(void)   //存放数据地址从1开始
{
	uint16_t i;
	uint8_t N,j;
	if (Run_Count<(u16)1)
...
{
	uint16_t a=0,b=0,c=0;//出气口温度达到设定温度-1
...
void SaveDateToFlashFunc(void)
{
	...
	else  if(Enable_Save_Data==(uint8_t)3)//存放数据
	{
		...
		if(RTD_Sensor_Err!=(uint8_t)0) 
		{
			SaveData[7]|=(uint8_t)0x20;
		}
		if(CQK_Sensor_Err!=(uint8_t)0)
		{
			SaveData[7]|=(uint8_t)0x40;
		}			
...
//	Disp_Err=0;		
	...
	if(No_ReadData_SHIDU_Times>(uint8_t)10)//人体端湿度传感器错误检测
	{
		SHIDU_Sensor_Err=(uint8_t)1;
		RT_SHIDU = 0; 
	}		
...
//有发热丝时的患者端温度控制Controler算法---根据患者端目标温度和实际温度，计算出加热回路驱动值  每1秒工作一次
static void Controler_Calc(void) //设定温度最高400,最低000,实际0-800,  
{

	INT Vlaue1,tmp;
	static const INT Controler_Kp = 40;//比例系数 5度为1500;1度为300;0.1度为30
	static const INT Controler_Ki = 8;//积分系数
	static const INT Controler_Kd = 20;//微分系数 	

	if(Controler_temp.Ek > 100)//10度为极限 
	{
	...
	if(Controler_temp.Ek < -100)
	{
 	...
 	Controler_temp.Sum_error += Controler_temp.Ek;//误差总量
 	if(Controler_temp.Sum_error > 3500) //0.1度内最多是250一周期
 	{
	...
	Vlaue1 = Controler_Kp * Controler_temp.Ek;//比例 最大100*30=3000;  误差*比例系数(假定相差5度即50*25=1250)
	...
	Controler_temp.Uk /= 4;//最高190; 	
		
 	Controler_temp.Ek2 = Controler_temp.Ek1;        // 保存新的K-1次输入值
 	Controler_temp.Ek1 = Controler_temp.Ek;
}


//加热盘驱动的Controler算法--- 根据加热盘的目标温度和实际温度，计算出加热回路驱动值  每1秒工作一次
static void HeatPlate_Controler_Calc(void) //设定温度最高400,最低000,实际0-800,  
{

 	INT Vlaue1,tmp;
//	uint16_t  Val_Calc;
 	static const INT Controler_Kp2 = 38;//比例系数 //比例系数 10度为4000/20=200;1度为400/20=20;0.1度为2
 	static const INT Controler_Ki2 = 8;//积分系数
 	static const INT Controler_Kd2 = 20;//微分系数 	

	if(Controler_temp2.Ek > 100)//10度为极限 
	{
	...
	if(Controler_temp2.Ek < -100)
	{
 	...
 	Controler_temp2.Sum_error += Controler_temp2.Ek;//误差总量
 	if(Controler_temp2.Sum_error > 3500) //0.1度内最多是250一周期
 	{
	...
	Vlaue1 = Controler_Kp2 * Controler_temp2.Ek;//比例 最大100*30=3000;  误差*比例系数(假定相差5度即50*25=1250)
	...
	Controler_temp2.Uk /= 4;//最高195; 		

 	Controler_temp2.Ek2 = Controler_temp2.Ek1;        // 保存新的K-1次输入值
 	Controler_temp2.Ek1 = Controler_temp2.Ek;
}


//加热盘目标温度的Controler算法--- 根据出气口的目标温度和实际温度，计算出加热盘目标温度的偏差, 每20秒工作一次
static void HeatPlateTemp_Aim_Controler_Calc(void) // 
{
 	INT Vlaue1,tmp;
//	uint16_t  Val_Calc;
 	static const INT Controler_Kp3 = 40;//比例系数 10度为4000/100=40;1度为400/100=4;0.1度为0.4
 	static const INT Controler_Ki3 = 1;//积分系数
 	static const INT Controler_Kd3 = 80;//微分系数 	

	if(Controler_temp3.Ek > 100)//10度为极限 
	{
	...
	if(Controler_temp3.Ek < -100)
	{
 	...
 	Controler_temp3.Sum_error += Controler_temp3.Ek;//误差总量
 	if(Controler_temp3.Sum_error > 1000) //0.1度内最多是250一周期
 	{
	...
	Vlaue1 = Controler_Kp3 * Controler_temp3.Ek;//比例 最大100*40=4000;  误差*比例系数(假定相差5度即50*25=1250)
	...
	Controler_temp3.Uk /= 100;//最高50; 		

 	Controler_temp3.Ek2 = Controler_temp3.Ek1;        // 保存新的K-1次输入值
...
static void PatientTemp_NoneHeatWire_Adj(void)
{
...

	{
		...
		{
			//Micro_Temp_Val = 190;
			//以下为发热盘温度的Controler控制
			
			Controler_temp2.Ek = 980 - JEP_Temp;//当前的误差量 设定温度-当前实际温度
			HeatPlate_Controler_Calc(); 
			Val_Calc = (uint8_t)Controler_temp2.Uk;	//计算出加热时间
			Micro_Temp_Val = (uint8_t)Val_Calc;//便于计算，以免超过190进入中断	
		...
		{	
			...
			{	
				...
				{
					err_sum=-180;//每秒10度(100)，30000/100=300,即为5分钟数据
				}
				else 
				{
					//do nothing
				...
				{	//8表示高位为0也显示,0表示非叠加显示 出气口
					...
					if(err_sum >=0)
					{
						LCD_ShowxNum(220,10,16,1,1,0x80,BLACK18); //1-表示为正数
					}
					else
					{
						LCD_ShowxNum(220,10,16,1,0,0x80,BLACK18); //0-表示为负数
					}						
					LCD_ShowxNum(220,20,16,5,(u32)(abs(err_sum)),0x80,BLACK18); //发热盘加热的模式显示
				}						
			...
			//以下为发热盘温度的Controler控制
			Controler_temp2.Ek = Temp2_Int - JEP_Temp;//当前的误差量 设定温度-当前实际温度
			HeatPlate_Controler_Calc(); 
			Val_Calc = (uint8_t)Controler_temp2.Uk;	//计算出加热时间
