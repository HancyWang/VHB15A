...
//	Temp_Dis_En = 0;//温度显示标志
	...
	else if(((Bit_is_one(ERR_Kind,Alarm_Const_LoTemp))&&((INT)Display_Temp_Kind==0))  //患者端低温红色
				 ||((LoTemp_CQK_Count>120)&&((INT)Display_Temp_Kind==1)))  //出气口低温红色
	{
		Temp_Dis_En = 1;//温度显示标志
		color=RED18;
		if(Alarm_WenDu_Cnt == 1)
