	...
	else if(Key_State_Present == KEY_STATE_OK_Short)//确认
	{
		...
		if(Set_RT_Temp != (INT)Remeber_Temp_Value)//若未改变人体端温度设定则不进行操作
		{
			...
			if(Work_Mode == Noninvasive_Mode)
			{
				...
				data_flash.Set_RT_WCTemp = (uint8_t)(Set_RT_WCTemp/5);//无创设定温度 
			...
			else
			{
				...
				data_flash.Set_RT_YCTemp = (uint8_t)(Set_RT_YCTemp/5);//有创设定温度 
				
				if(Set_RT_YCTemp >= 370) 
				{
					Set_CQK_YCTemp = Set_RT_YCTemp - 30;//人体端>=37时出气口温度为RT-3
				}
				else
				{
	...
	else if(Key_State_Present == KEY_STATE_OK_Short)//确认
	{
		...
		if(Set_CQK_Temp != Remeber_Temp_Value)//若未改变人体端温度设定则不进行操作
		{
			...
			if(Work_Mode==Noninvasive_Mode)
			{
				...
				data_flash.Set_CQK_WCTemp = (uint8_t)(Set_CQK_WCTemp/5);//无创设定温度 																		
			}
			else
			{
				Set_CQK_YCTemp=Set_CQK_Temp; 
				data_flash.Set_CQK_YCTemp = (uint8_t)(Set_CQK_YCTemp/5);//有创设定温度 																					
			}	
